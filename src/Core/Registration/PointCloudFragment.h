...
{
public:
    PointCloudFragment(int index = 0, int resolution = 8, float length = 3.0f);
    ~PointCloudFragment(void);
...
public:
    ...
    inline void UpdateNormal(const Eigen::VectorXd & ctr, Point & point) {
        for (int i = 0; i < 3; i++) {
            point.n_[i] = 0.0f;
            for (int j = 0; j < 8; j++) {
                point.n_[i] += point.nval_[j] *
                        (float)ctr(point.idx_[j] + i + offset_);
            }
        }
        float len = sqrt(point.n_[0] * point.n_[0] +
                point.n_[1] * point.n_[1] +
                point.n_[2] * point.n_[2]);
        point.n_[0] /= len;
        point.n_[1] /= len;
        point.n_[2] /= len;
    }
    ...
    inline void UpdatePose(const Eigen::Matrix4f & inc_pose) {
        ...
        for (int i = 0; i < (int)points_.size(); i++) {
            ...
            p = inc_pose * Eigen::Vector4f(
                    point.p_[0], point.p_[1], point.p_[2], 1);
            n = inc_pose * Eigen::Vector4f(
                    point.n_[0], point.n_[1], point.n_[2], 0);
            point.p_[0] = p(0);
            point.p_[1] = p(1);
            point.p_[2] = p(2);
            point.n_[0] = n(0);
    ...
    inline Eigen::Vector3d UpdatePoint(
            const Eigen::VectorXd & ctr, Point & point) {
        Eigen::Vector3d pos;
        for (int i = 0; i < 3; i++) {
            pos(i) = 0.0;
            for (int j = 0; j < 8; j++) {
                pos(i) += point.val_[j] * (float)ctr(point.idx_[j]
                        + i + offset_);
            }
        }
        return pos;
