...
static RGBDTrajectory rgbd_traj_;
static RGBDTrajectory reg_traj_;
...
static double length_;
static int max_iteration_;
static int max_inner_iteration_;
static std::string dir_prefix_;
static std::string rgbd_filename_;
static std::string reg_filename_;
static std::string ctr_filename_;
static std::string sample_filename_;
static std::string init_ctr_file_;
static std::string pose_filename_;
static int sample_num_;
static int blacklist_pair_num_;

// hidden parameters
static double unit_length_;
static int nper_;
static std::vector< Eigen::Matrix4d > ipose_;
static int matrix_size_;

// global point cloud, let's trust the memory size ...
static std::vector< PointCloudFragment > pointclouds_;

// global correspondences
static std::vector< Correspondence > corres_;

// black listed fragments
static std::unordered_set< int > blacklist_;
static std::vector< int > absolute2relative_map_;
static std::vector< int > relative2absolute_map_;

// rigid
static std::vector< Eigen::Matrix4d > pose_;
static RGBDTrajectory output_traj_;

// slac
static std::vector< Eigen::Matrix3d > pose_rot_t_;
...
void InitMap()
{
    ...
    if (num_ != relative2absolute_map_.size()) {
        three::PrintWarning("Blacklisted fragments, number reduced from %d to %d.\n",
...
void InitIPose()
{
    ...
    Eigen::Matrix4d leftbase = basepose *
            rgbd_traj_.data_[0].transformation_.inverse();
    ipose_.resize(num_);
    pose_.resize(num_);
    ...
    for (int i = 0; i < num_; i++) {
        ...
        ipose_[i] = leftbase *
                rgbd_traj_.data_[ii*interval_].transformation_ *
                baseinverse;
    }
    three::PrintInfo("IPose initialized.\n");
}
...
void InitPointClouds()
{
    pointclouds_.clear();
    for (int i = 0; i < num_; i++) {
        pointclouds_.push_back(PointCloudFragment(
                i, resolution_, length_));
    }
    // why parallel for is not working?
...
//            // } else {
//                sprintf(filename, "%sfrag_%03d.ply", dir_prefix_.c_str(), ii);
//                // if (boost::filesystem::exists(std::string(filename))) {
//                    pointclouds_[i].LoadFromPLYFile(filename);
//                // } else {
//                    three::PrintError("File not found ... Check dir and num parameters.\n");
//                // }
//            // }
//        // }
//    }
...
void InitCorrespondences()
{
    ...
    for (int i = 0; i < (int)reg_traj_.data_.size(); i++) {
        if (blacklist_.find(reg_traj_.data_[i].id1_) != blacklist_.end() ||
                blacklist_.find(reg_traj_.data_[i].id2_) != blacklist_.end() ||
                reg_traj_.data_[i].id1_ >= num_ ||
                reg_traj_.data_[i].id2_ >= num_) {
            continue;
        }
        if (reg_traj_.data_[i].frame_ != -1 &&
                reg_traj_.data_[i].frame_ >= blacklist_pair_num_) {
            corres_.push_back(Correspondence(
                    absolute2relative_map_[reg_traj_.data_[i].id1_],
                    absolute2relative_map_[reg_traj_.data_[i].id2_]));
            char filename[1024];
            memset(filename, 0, 1024);
            sprintf(filename, "%scorres_%d_%d.txt", dir_prefix_.c_str(),
                    reg_traj_.data_[i].id1_, reg_traj_.data_[i].id2_);
            corres_.back().trans_ = reg_traj_.data_[i].transformation_;
            corres_.back().LoadFromFile(filename);
            three::PrintInfo("Read %s, get %d correspondences.\n",
    ...
    if (init_ctr_file_.length() > 1) {
        ...
        if (f != NULL) {
            ...
            for (int i = 0; i < num_ * nper_ / 3; i++) {
                fgets(buffer, 1024, f);
                sscanf(buffer, "%lf %lf %lf", &ctr(i * 3),
                        &ctr(i * 3 + 1), &ctr(i * 3 + 2));
            }
            fclose(f);
        }
            ...
            for (int i = 0; i <= resolution_; i++) {
                for (int j = 0; j <= resolution_; j++) {
                    for (int k = 0; k <= resolution_; k++) {
                        Eigen::Vector4d pos(i * unit_length_,
                                j * unit_length_, k * unit_length_, 1);
                        Eigen::Vector4d ppos = ipose_[l] * pos;
                        ctr(l * nper_ + pointclouds_[0].GetIndex(i, j, k) * 3 + 0) = ppos(0);
                        ctr(l * nper_ + pointclouds_[0].GetIndex(i, j, k) * 3 + 1) = ppos(1);
                        ctr(l * nper_ + pointclouds_[0].GetIndex(i, j, k) * 3 + 2) = ppos(2);
                    }
                }
            }
        }
    ...
    for (int i = 0; i <= resolution_; i++) {
        for (int j = 0; j <= resolution_; j++) {
            for (int k = 0; k <= resolution_; k++) {
                ...
                ctr(num_ * 6 + pointclouds_[0].GetIndex(i, j, k) * 3 + 0) = pos(0);
                ctr(num_ * 6 + pointclouds_[0].GetIndex(i, j, k) * 3 + 1) = pos(1);
                ctr(num_ * 6 + pointclouds_[0].GetIndex(i, j, k) * 3 + 2) = pos(2);
            }
        }
    }
    if (init_ctr_file_.length() > 1) {
        ...
        if (f != NULL) {
            ...
            for (int i = 0; i < nper_ / 3; i++) {
                ...
                sscanf(buffer, "%lf %lf %lf",
                        &thisCtr(num_ * 6 + i * 3),
                        &thisCtr(num_ * 6 + i * 3 + 1),
                        &thisCtr(num_ * 6 + i * 3 + 2));
            }
            fclose(f);
        }
    } else {
...
void InitBaseAA(SparseMatrix & baseAA)
{
    ...
    for (int l = 0; l < num_; l++) {
        ...
        for (int i = 0; i <= resolution_; i++) {
            for (int j = 0; j <= resolution_; j++) {
                for (int k = 0; k <= resolution_; k++) {
                    int idx[2] = { pointclouds_[0].GetIndex(i, j, k) * 3, 0 };
                    double val[2] = { 1, -1 };
                    if (i > 0) {
                        idx[1] = pointclouds_[0].GetIndex(i - 1, j, k) * 3;
                    ...
                    if (j > 0) {
                        idx[1] = pointclouds_[0].GetIndex(i, j - 1, k) * 3;
                        mat.AddHessian2(idx, val, tri);
                    }
                    if (j < resolution_) {
                        idx[1] = pointclouds_[0].GetIndex(i, j + 1, k) * 3;
                    ...
                    if (k > 0) {
                        idx[1] = pointclouds_[0].GetIndex(i, j, k - 1) * 3;
                        mat.AddHessian2(idx, val, tri);
                    }
                    if (k < resolution_) {
                        idx[1] = pointclouds_[0].GetIndex(i, j, k + 1) * 3;
...
void InitBaseJJ(SparseMatrix & baseJJ)
{
    ...
    int base_anchor = pointclouds_[0].GetIndex(resolution_ / 2, resolution_ / 2, 0) * 3;
...
Eigen::Matrix3d GetRotation(const int idx, const std::vector< int > & idxx,
        const Eigen::VectorXd & ictr, const Eigen::VectorXd & ctr)
{
    int n = (int)idxx.size();
    Eigen::Matrix3d C = Eigen::Matrix3d::Zero();

    for (int i = 0; i < n; i++) {
        Eigen::RowVector3d dif(ictr(idx) - ictr(idxx[i]),
            ictr(idx + 1) - ictr(idxx[i] + 1),
            ictr(idx + 2) - ictr(idxx[i] + 2));
        Eigen::RowVector3d diff(ctr(idx) - ctr(idxx[i]),
            ctr(idx + 1) - ctr(idxx[i] + 1),
            ctr(idx + 2) - ctr(idxx[i] + 2));
...
{
    three::PrintInfo("Save ctr to file %s ... ", filename.c_str());
...
void SavePoints(const Eigen::VectorXd & ctr, std::string filename)
{
    ...
    //         pt.normal_y = norm(1);
    //         pt.normal_z = norm(2);
    //         pt.r = pointclouds_[l].points_[i].rgb_[0];
    //         pt.g = pointclouds_[l].points_[i].rgb_[1];
    //         pt.b = pointclouds_[l].points_[i].rgb_[2];
    //         pcd.push_back(pt);
    //     }
    // }
    ...
    for (int l = 0; l < num_; l++) {
        pointclouds_[l].UpdateAllNormal(ctr);
        for (int i = 0; i < (int)pointclouds_[l].points_.size(); i += sample_num_) {
            Eigen::Vector3d pos = pointclouds_[l].UpdatePoint(
                    ctr, pointclouds_[l].points_[i]);
            Eigen::Vector3d norm(
                    pointclouds_[l].points_[i].n_[0],
                    pointclouds_[l].points_[i].n_[1],
                    pointclouds_[l].points_[i].n_[2]);
            norm.normalize();
            Eigen::Vector3d xyz, normal, rgb;
            xyz << pos(0), pos(1), pos(2);
            normal << norm(0), norm(1), norm(2);
            rgb << pointclouds_[l].points_[i].rgb_[0],
                    pointclouds_[l].points_[i].rgb_[1],
                    pointclouds_[l].points_[i].rgb_[2];
            pcd.points_.push_back(xyz);
            pcd.normals_.push_back(normal);
            pcd.colors_.push_back(rgb);
        }
    }

    three::PrintInfo("Save sample pcd into %s ... ", filename.c_str());
    // pcl::PCDWriter w;
    // w.writeBinaryCompressed(filename, pcd);
    bool write_ascii = false;
    bool compressed = true;
    WritePointCloud(filename, pcd, write_ascii, compressed);
}
...
void ExpandCtr(const Eigen::VectorXd & ctr, Eigen::VectorXd & expand_ctr)
{
    ...
    for (int l = 0; l < num_; l++) {
        for (int i = 0; i < nper_ / 3; i++) {
            Eigen::Vector4d pos = pose_[l] * Eigen::Vector4d(
                    ctr(num_ * 6 + i * 3 + 0),
                    ctr(num_ * 6 + i * 3 + 1),
                    ctr(num_ * 6 + i * 3 + 2), 1);
...
{
    three::PrintWarning("Nonrigid optimization.\n");
    three::PrintInfo("Parameters: weight %.5f, resolution %d, piece number %d, max iteration %d\n",
            weight_, resolution_,
            num_, max_iteration_);

    InitMap();
    unit_length_ = length_ / resolution_;
    nper_ = (resolution_ + 1) * (resolution_ + 1) *
            (resolution_ + 1) * 3;
    ...
    for (int itr = 0; itr < max_iteration_; itr++) {
        ...
        thisAA = baseAA;
        three::PrintInfo("Processing %d : %5d", corres_.size(), 0);
...
#pragma omp critical
            {
                nprocessed++;
                thisAA += tempAA;
                three::PrintInfo("\b\b\b\b\b%5d", nprocessed);
            }
        }
        three::PrintInfo(" ... Done.\n");
        ...
        for (int m = 0; m < max_inner_iteration_; m++) {
            ...
            for (int l = 0; l < num_; l++) {
                for (int i = 0; i <= resolution_; i++) {
                    for (int j = 0; j <= resolution_; j++) {
                        for (int k = 0; k <= resolution_; k++) {
                            int idx = pointclouds_[0].GetIndex(i, j, k) * 3 + l * nper_;
                            std::vector< int > idxx;
                            if (i > 0) {
                                idxx.push_back(pointclouds_[0].GetIndex(i - 1, j, k) * 3 + l * nper_);
                            }
                            if (i < resolution_) {
                                idxx.push_back(pointclouds_[0].GetIndex(i + 1, j, k) * 3 + l * nper_);
                            }
                            if (j > 0) {
                                idxx.push_back(pointclouds_[0].GetIndex(i, j - 1, k) * 3 + l * nper_);
                            }
                            if (j < resolution_) {
                                idxx.push_back(pointclouds_[0].GetIndex(i, j + 1, k) * 3 + l * nper_);
                            }
                            if (k > 0) {
                                idxx.push_back(pointclouds_[0].GetIndex(i, j, k - 1) * 3 + l * nper_);
                            }
                            if (k < resolution_) {
                                idxx.push_back(pointclouds_[0].GetIndex(i, j, k + 1) * 3 + l * nper_);
                            }
            ...
            three::PrintWarning("Iteration #%d:%d (%d:%d) : score is %.4f\n",
                    itr + 1, m + 1, max_iteration_,
                    max_inner_iteration_, score);
...
void OptimizeRigid()
{
    three::PrintWarning("Rigid optimization.\n");
    three::PrintInfo("Parameters: resolution %d, piece number %d, max iteration %d\n", resolution_, num_, max_iteration_);
...
#pragma omp critical
        ...
        three::PrintInfo(" ... Done.\n");
        three::PrintInfo("Error score is : %.2f\n", thisscore);
        ...
        for (int l = 0; l < num_; l++) {
            ...
            aff_mat.linear() = (Eigen::Matrix3d) Eigen::AngleAxisd(
                        result(l * 6 + 2), Eigen::Vector3d::UnitZ())
                        * Eigen::AngleAxisd(
                        result(l * 6 + 1), Eigen::Vector3d::UnitY())
                        * Eigen::AngleAxisd(
                        result(l * 6 + 0), Eigen::Vector3d::UnitX());
    ...
    for (int i = 0; i < (int)pose_.size(); i++) {
        output_traj_.data_.push_back(
                FramedTransformation(i, i, i + 1, pose_[i]));
    }
...
void OptimizeRigidP2P()
{
    three::PrintWarning("Rigid optimization with point to point distance.\n");
    three::PrintInfo("Parameters: resolution %d, piece number %d, max iteration %d\n",
            resolution_, num_, max_iteration_);
    ...
    rgbd_traj_.data_.clear();
    for (int i = 0; i < (int)ipose_.size(); i++) {
        rgbd_traj_.data_.push_back(
                FramedTransformation(i, i, i + 1, ipose_[i]));
        pose_[i] = ipose_[i];
        pointclouds_[i].UpdatePose(pose_[i].cast< float >());
    }
...
#pragma omp parallel for num_threads(8) schedule(dynamic)
        for (int l = 0; l < (int)corres_.size(); l++) {
            ...
            for (int k = 0; k < (int)corres_[l].corres_.size(); k++) {
                ...
                val[4] = ppj(1);
                val[5] = -1;
                mat_adder.AddHessian(idx, val, 6, tri);
                mat_adder.AddJb(idx, val, 6, ppi(0) - ppj(0), tempJb);
                ...
                val[5] = -1;
                mat_adder.AddHessian(idx, val, 6, tri);
                mat_adder.AddJb(idx, val, 6, ppi(1) - ppj(1), tempJb);
                ...
                val[5] = -1;
                mat_adder.AddHessian(idx, val, 6, tri);
                mat_adder.AddJb(idx, val, 6, ppi(2) - ppj(2), tempJb);
            }
            tempJJ.setFromTriplets(tri.begin(), tri.end());
...
#pragma omp critical
        ...
        three::PrintInfo(" ... Done.\n");
        three::PrintInfo("Error score is : %.2f\n", thisscore);
        three::PrintInfo("Reference error score is : %.2f\n", refscore);
...
void OptimizeSLAC()
{
    three::PrintWarning("SLAC optimization.\n");
    three::PrintInfo("Parameters: weight %.5f, resolution %d, piece number %d, max iteration %d\n",
...
#pragma omp critical
            ...
            three::PrintInfo(" ... Done.\n");
            three::PrintInfo("Data error score is : %.2f\n", thisscore);
            ...
            for (int i = 0; i <= resolution_; i++) {
                for (int j = 0; j <= resolution_; j++) {
                    for (int k = 0; k <= resolution_; k++) {
                        int idx = pointclouds_[0].GetIndex(i, j, k) * 3 + 6 * num_;
                        std::vector<int> idxx;
                        if (i > 0) {
                            idxx.push_back(pointclouds_[0].GetIndex(i - 1, j, k) * 3 + 6 * num_);
                        }
                        if (i < resolution_) {
                            idxx.push_back(pointclouds_[0].GetIndex(i + 1, j, k) * 3 + 6 * num_);
                        }
                        if (j > 0) {
                            idxx.push_back(pointclouds_[0].GetIndex(i, j - 1, k) * 3 + 6 * num_);
                        }
                        if (j < resolution_) {
                            idxx.push_back(pointclouds_[0].GetIndex(i, j + 1, k) * 3 + 6 * num_);
                        }
                        if (k > 0) {
                            idxx.push_back(pointclouds_[0].GetIndex(i, j, k - 1) * 3 + 6 * num_);
                        }
                        if (k < resolution_) {
                            idxx.push_back(pointclouds_[0].GetIndex(i, j, k + 1) * 3 + 6 * num_);
                        }
                        ...
                        for (int t = 0; t < (int)idxx.size(); t++) {
                            Eigen::Vector3d bx = Eigen::Vector3d(
                                    tempCtr(idx) - tempCtr(idxx[t]),
                                    tempCtr(idx + 1) - tempCtr(idxx[t] + 1),
                                    tempCtr(idx + 2) - tempCtr(idxx[t] + 2))
                                    - R * Eigen::Vector3d(
                                    ictr(idx) - ictr(idxx[t]),
                                    ictr(idx + 1) - ictr(idxx[t] + 1),
                                    ictr(idx + 2) - ictr(idxx[t] + 2));
            ...
            three::PrintInfo("Regularization error score is : %.2f\n", regscore);
...
void OptimizeSLACP2P()
{
    three::PrintWarning("SLAC optimization with point to point distance.\n");
    three::PrintInfo("Parameters: weight %.5f, resolution %d, piece number %d, max iteration %d\n", weight_, resolution_, num_, max_iteration_);
...
#pragma omp parallel for schedule(dynamic)
            for (int l = 0; l < (int)corres_.size(); l++) {
                ...
                for (int k = 0; k < (int)corres_[l].corres_.size(); k++) {
                    ...
                    Eigen::Vector3d npj(pj.n_[0], pj.n_[1], pj.n_[2]);
                    Eigen::Vector3d diff = ppi - ppj;
                    score += diff.squaredNorm();
                    nscore += diff.dot(npi) * diff.dot(npi);
                    ...
                    for (int ll = 0; ll < 8; ll++) {
                        for (int xyz = 0; xyz < 3; xyz++) {
                            idx[6 + ll * 3 + xyz] = 6 * num_ + pi.idx_[ll] + xyz;
                            val[6 + ll * 3 + xyz] = pi.val_[ll] * pose_rot_t_[i](xyz, 0);
                        }
                    }
                    for (int ll = 0; ll < 8; ll++) {
                        for (int xyz = 0; xyz < 3; xyz++) {
                            idx[6 + 24 + ll * 3 + xyz] = 6 * num_ + pj.idx_[ll] + xyz;
                            val[6 + 24 + ll * 3 + xyz] = pj.val_[ll] * (- pose_rot_t_[j](xyz, 0));
                        }
                    }
                    for (int i = 0; i < buck_size; i++) {
                        tempJJ(idx[i], idx[i]) += val[i] * val[i];
                    ...
                    for (int ll = 0; ll < 8; ll++) {
                        for (int xyz = 0; xyz < 3; xyz++) {
                            idx[6 + ll * 3 + xyz] = 6 * num_ + pi.idx_[ll] + xyz;
                            val[6 + ll * 3 + xyz] = pi.val_[ll] * pose_rot_t_[i](xyz, 1);
                        }
                    }
                    for (int ll = 0; ll < 8; ll++) {
                        for (int xyz = 0; xyz < 3; xyz++) {
                            idx[6 + 24 + ll * 3 + xyz] = 6 * num_ + pj.idx_[ll] + xyz;
                            val[6 + 24 + ll * 3 + xyz] = pj.val_[ll] * (- pose_rot_t_[j](xyz, 1));
                        }
                    }
                    for (int i = 0; i < buck_size; i++) {
                        tempJJ(idx[i], idx[i]) += val[i] * val[i];
                    ...
                    for (int ll = 0; ll < 8; ll++) {
                        for (int xyz = 0; xyz < 3; xyz++) {
                            idx[6 + ll * 3 + xyz] = 6 * num_ + pi.idx_[ll] + xyz;
                            val[6 + ll * 3 + xyz] = pi.val_[ll] * pose_rot_t_[i](xyz, 2);
                        }
                    }
                    for (int ll = 0; ll < 8; ll++) {
                        for (int xyz = 0; xyz < 3; xyz++) {
                            idx[6 + 24 + ll * 3 + xyz] = 6 * num_ + pj.idx_[ll] + xyz;
                            val[6 + 24 + ll * 3 + xyz] = pj.val_[ll] * (- pose_rot_t_[j](xyz, 2));
                        }
                    }
                    for (int i = 0; i < buck_size; i++) {
                        tempJJ(idx[i], idx[i]) += val[i] * val[i];
...
#pragma omp critical
            ...
            three::PrintInfo(" ... Done.\n");
            three::PrintInfo("Data error score is : %.2f\n", thisscore);
            three::PrintInfo("Reference data error score is : %.2f\n", refscore);
...
{
    resolution_ = option.resolution_;
    interval_ = option.interval_;
    num_ = option.num_;
    weight_ = option.weight_;
    length_ = option.length_;
    max_iteration_ = option.max_iteration_;
    max_inner_iteration_ = option.max_inner_iteration_;
    dir_prefix_ = option.dir_prefix_;
    rgbd_filename_ = option.rgbd_filename_;
    reg_filename_ = option.reg_filename_;
    ctr_filename_ = option.ctr_filename_;
    sample_filename_ = option.sample_filename_;
    init_ctr_file_ = option.init_ctr_file_;
    pose_filename_ = option.pose_filename_;
    sample_num_ = option.sample_num_;
    blacklist_pair_num_ = option.blacklist_pair_num_;

    std::string blacklist_file, ipose_file;

    if (rgbd_filename_ != "") {
        rgbd_traj_.LoadFromFile(rgbd_filename_);
    }
    if (reg_filename_ != "") {
        reg_traj_.LoadFromFile(reg_filename_);
    }
    ...
    //}
    if (option.type_ == FragmentOptimizerType::SLAC) {
        OptimizeSLAC();
    } else if (option.type_ == FragmentOptimizerType::Rigid) {
        OptimizeRigid();
    } else if (option.type_ == FragmentOptimizerType::RigidP2P) {
        OptimizeRigidP2P();
    } else if (option.type_ == FragmentOptimizerType::SLACP2P) {
        OptimizeSLACP2P();
    } else if (option.type_ == FragmentOptimizerType::Nonrigid) {
        OptimizeNonrigid();
    }
    return 0;
}
